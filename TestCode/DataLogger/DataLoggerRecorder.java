...
public class DataLoggerRecorder {
		...
		try {
			...
			if (data.charAt(0) == 'e') {
				if (logFileWriter == null) {
					...
					System.out.println("Starting new files due to E being recieved");
					needNewLogFile = true;
                    needNewMessagesFile = true;
				}
			}
	...
	private static void newLogFile() {
		...
		try {
			...
			fNew = new File("Log_"+dateFormat.format(date) + ".csv");
	...
	private static void newMessageFile () {
		...
		try {
			date = new Date();
			SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH-mm-ss");
			fNew = new File("Messages_"+dateFormat.format(date) + ".txt");
			fwNew = new FileWriter(fNew, true);
		...
		saveData("mBeginning logging of messages");
