...
public class StrafeForDistance extends Command {
	 ...
	 * @param arg_distance Distance in encoder counts, negative is left, positive is right
	 */
	public StrafeForDistance(double arg_targetSpeed, StrafeDistanceUnits encoderTicks, double arg_distance) {
		requires(Robot.drive);
		
		if (encoderTicks == StrafeDistanceUnits.INCHES) {
			arg_distance = arg_distance / Drive.DISTANCE_PER_PULSE_MIDDLE;
		}
	...
	protected void initialize() {
		m_startPos = Robot.drive.getMiddleEncoder();
	...
	protected void execute() {
		Robot.drive.hDrive(0, 0, m_targetPower, false, false);
	}

	// Make this return true when this Command no longer needs to run execute()
	protected boolean isFinished() {
		int displacement = Robot.drive.getMiddleEncoder() - m_startPos;
